...
const countries = [
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1504150558240-0b4fd8946624?q=80&w=2864&auto=format&fit=crop"
  },
  {
    name: "Business Visa",
...
const CountryCarousel: React.FC = () => {
  ...
  return (
    <div className="bg-[#013D2F] py-20 rounded-[48px] mx-4 sm:mx-8 relative overflow-hidden">
      ...
      <div className="px-6 md:px-12 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-[#84cc16] font-semibold text-xs tracking-widest uppercase">
          ...
          <div className="flex gap-4">
            <button 
                onClick={prevSlide}
                className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-[#84cc16] hover:border-[#84cc16] transition-all"
            >
                <ArrowLeft size={20} />
            </button>
            <button 
                onClick={nextSlide}
                className="w-12 h-12 rounded-full border bg-[#84cc16] border-[#84cc16] flex items-center justify-center text-[#013D2F] hover:bg-white hover:text-[#013D2F] transition-all"
            >
                <ArrowRight size={20} />
            </button>
          </div>
          ...
          {countries.map((country, index) => {
             ...
             return (
                <div 
                    key={index}
                    className={`
                        min-w-[280px] md:min-w-[320px] h-[400px] rounded-3xl overflow-hidden relative transition-all duration-500 ease-in-out cursor-pointer
                        ${isActive ? 'w-[320px] ring-4 ring-[#84cc16]/50' : 'opacity-60 grayscale hover:grayscale-0'}
                    `}
                ...
                >
                    ...
                    {/* Active Card Overlay */}
                    {isActive ? (
                        <div className="absolute inset-0 bg-[#84cc16]/90 p-8 flex flex-col justify-center text-[#013D2F]">
